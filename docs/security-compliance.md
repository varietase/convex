...
> **Scope:** Public hackathon deployment with bounded public repositories, ephemeral learner responses, no source-code writes, and a final-product client shell for public-link plus MCP-placeholder repository connection.
...
| MCP repository list placeholder | Demo/internal placeholder | Browser memory in centralized client state | No provider token; replace with local MCP host contract before private-repo support |
...
MVP backend does not solicit names, email, accounts, private repositories, or repository credentials. The current frontend may show an MCP connection placeholder for private/local repository access, but it does not authenticate with GitHub or access private code. Public code may still contain accidentally committed secrets; convex treats all source as sensitive in handling and never includes unrelated source in model evidence packets.
...
- **Repository authority:** backend MVP authority is only anonymous read of allowlisted public HTTPS hosts or bundled sample IDs; no OAuth, SSH keys, write token, git push, or provider mutation API (INV-003). The client MCP shell is placeholder-only until the local MCP host contract exists.
- **Future MCP authority:** private/local repository discovery SHALL be mediated by the local MCP host. The frontend SHALL NOT perform GitHub OAuth, store provider tokens, or call GitHub directly for private repository discovery.
...
| T-013 | Elevation | MCP placeholder or future MCP caller gains write/tool access | Current placeholder exposes no host tools; future threat review; sidecar tools read-only and no shell/editor mutation authority | INV-003 |
...
- Repository intake receives no provider token in MVP. Any browser code path requesting GitHub write/private-repo OAuth, storing provider tokens, or bypassing the local MCP host fails security review.
...
- [ ] INV-003 static/runtime checks confirm no write token, repository mutation API, code execution, commit, PR path, browser OAuth flow, or provider-token storage. — 2026-07-18
