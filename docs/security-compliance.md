...
> **Scope:** Public hackathon deployment with bundled/public repositories, ephemeral learner responses, no source-code writes, and a public-link client repository connection shell. MCP/private/local repository connection is removed from the current build.
...
MVP backend does not solicit names, email, accounts, private repositories, or repository credentials. The current frontend does not show private/local repository access, GitHub OAuth, or provider-token entry. Public code may still contain accidentally committed secrets; convex treats all source as sensitive in handling and never includes unrelated source in model evidence packets.
...
- **Repository authority:** backend MVP authority is only anonymous read of allowlisted public HTTPS hosts or bundled sample IDs; no OAuth, SSH keys, write token, git push, or provider mutation API (INV-003). There is no client private/local repository connector in the current build.
...
| T-013 | Elevation | Future private/local connector gains write/tool access | No current private/local connector; any future connector requires threat review and no shell/editor mutation authority | INV-003 |
...
- Repository intake receives no provider token in MVP. Any browser code path requesting GitHub write/private-repo OAuth, storing provider tokens, or presenting private/local repository discovery fails security review.
