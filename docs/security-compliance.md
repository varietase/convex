...
| Evidence graph and file paths | Public-derived / session-confidential | Space ephemeral store; BFF in transit | Session authorization; 24h TTL [assumption] |
| Teach-back response and learner state | User content / potentially personal | Space ephemeral store | Session authorization; 24h TTL [assumption] |
| Session cookie and CSRF secret | Sensitive auth data | Browser cookie / Vercel signing path | HttpOnly cookie; hash CSRF server-side [assumption] |
| Backend/model credentials | Secret | Vercel/HF secret stores | Never expose to browser or logs |
...
## Authn / authz model
- **Browser:** ephemeral signed session cookie created by API-001 [assumption]. Mutations also require CSRF token and allowlisted origin. Every resource lookup is scoped to `session_id`.
- **Vercel → Space:** server-held bearer credential on every API-101–API-108 request [assumption]. The Space rejects missing/invalid credentials before body processing.
- **Authorization:** session owns its analyses, graph, questions, attempts, and gaps. No admin or cross-session endpoint exists.
...
- Exact signing, secret-rotation, and host allowlists are implementation assumptions pending scaffold validation.


## Threat model
| T-ID | STRIDE | Vector / impact | Mitigation | Enforces |
|---|---|---|---|---|
| T-001 | Spoofing | Forged/guessed session accesses another graph or response | Signed HttpOnly cookie, opaque IDs, ownership checks, 404 on mismatch, short TTL | INV-002, BR-004 |
| T-002 | Spoofing | Browser impersonates BFF to Space | Server-held bearer credential, constant-time validation, rotation, browser denied direct credential | BR-010 |
...
| T-012 | Elevation | Client changes session/resource IDs or internal fields | BFF allowlist/schema validation; backend reconstructs evidence packets; session claim authorization | INV-001–003 |
...
| Unfair learner judgment | Learner | Evaluation treated as ability/mastery | Critique response only; categorical evidence; no score/mastery claim; Low confidence (INV-002) |
...
## Secrets handling
- Store `XRAY_BACKEND_TOKEN` and session-signing key in Vercel secrets; store backend token and `OPENAI_API_KEY` in Hugging Face runtime secrets.
...
- Use separate preview/production values [assumption]. Rotate on suspected exposure; redeploy both sides for backend token overlap; revoke old value; invalidate sessions on signing-key rotation.
...
## Audit & logging
Log: request ID, hashed ephemeral session ID, route/API ID, status, duration bucket, job stage, analyzer/rule/schema version, model request ID, error code, rate-limit event, deletion completion, and invariant violation. Do not log source text, full repository URL/query, file contents, learner response, model prompt/output body, cookies, CSRF values, auth headers, API keys, or evidence packet. Retain sanitized logs 7 days [assumption] and anonymous counters 30 days [assumption]. Audit derivations live only with the 24-hour session.

## Incident response basics
1. **Detect:** API-106/health, 5xx/401/rate/invariant counters, smoke test, user report.
...
4. **Recover:** deploy backend first, verify authenticated health/sample, deploy BFF, run full loop and negative security tests.
...
- [ ] API-001–API-108 auth/authz matches `api-spec.md`; no undocumented network route. — 2026-07-18
...
- [ ] Session isolation, CSRF/origin, expiration, and deletion tests pass. — 2026-07-18
- [ ] Logs inspected under success/error paths contain no source, response, prompt, cookie, or secret body. — 2026-07-18
