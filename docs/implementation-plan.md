...
|----|-----------------|------------|-------|-------------|----------|--------|-----------------|
| TASK-001 | Credential unblock: OpenAI GPT-5.6 key provided; HF write token no longer needed (pivot to AWS EC2); infra | — | Farhana | none (external credentials; no repo write) | — | done | `result: PASS — OpenAI API key verified; HF token requirement removed by EC2 pivot` |
...
| TASK-003 | Deploy backend to AWS EC2; /health green; keyed production_smoke ×3 (pivot from HF Space to EC2); F-005 | TASK-001 | Geinel | none (EC2 runtime config; no repo write) | — | done | `result: PASS — backend deployed on AWS EC2; /health responds; HF Space no longer used` |
