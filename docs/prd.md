...
- As a builder, I want to start with a bundled sample or public GitHub repository link so that I can reach the proof-of-comprehension loop without private-repository setup.
...
## User journeys
- **UJ-000 — Connect a repository:** A builder chooses the bundled sample or enters a valid public GitHub URL. Private and local repository access is removed from the current build.
...
| F-102 | In-workflow/private repository surface | Cut from current build | Private/local repository access and separate surfaces add workflow friction. | Removed from the current system for the hackathon timeline; revisit only after the public/sample proof-of-comprehension loop is verified. |
...
| F-104 | Agent teaching contract | Final | Coding agents lack a standard way to query which concepts need explanation. | Future read-only tool contract; deterministic graph remains structural truth. |
...
- **BR-011 — Public repository connection:** Current onboarding SHALL present only bundled sample intake and public GitHub repository links. Public GitHub repositories MAY be entered as `https://github.com/{owner}/{repo}`; private and local repository connection controls SHALL NOT appear in the current build.
- **BR-012 — No private-repository credentials:** The frontend SHALL NOT perform GitHub OAuth, store GitHub access tokens, request provider credentials, or present private/local repository discovery in the current build.
...
## User flows
### UF-000 — Public repository onboarding
...
3. The builder enters a public GitHub URL in the form `https://github.com/{owner}/{repo}`.
...
6. The modal does not show MCP, private repository, local workspace, OAuth, or token-entry controls.
7. If a builder cannot use a public repository, the demo-safe path is the bundled/pre-indexed sample rather than private/local repository connection.
8. Once a public repository or sample is selected, the dashboard shows repository status, sidebar sections, and placeholder workspace panels until analysis data is wired.

### UF-001 — Repository to evidence-backed path
1. The builder chooses the bundled sample or a supported bounded public repository snapshot. Private/local repository sources are out of scope for the current build.
...
- **F-005 / AC-017:** WHEN the repository-link modal is open, THEN MCP, private repository, local workspace, OAuth, and token-entry controls SHALL NOT be shown.
...
- **F-102 / AC-102:** WHERE private/local repository access is reconsidered after the current build, THEN it SHALL require a new decision, security review, tests, and the same read-only evidence-backed graph and teach-back loop.
- **F-102 / AC-105:** WHERE private/local repository access is reconsidered after the current build, THEN the browser SHALL still avoid provider-token storage and write-authority flows.
...
- Private repository analysis, local workspace analysis, broad language coverage, or production-scale multi-tenancy in MVP [assumption].
- F-101, F-102, F-103, and F-104 in the MVP build.

## Dependencies
### Current implementation baseline
- **Repository 1 — client:** Cloudflare Workers-hosted web client (OpenNext) implemented with Next.js App Router, React, TypeScript, Tailwind CSS, and CSS design tokens. The current client includes public GitHub URL validation, centralized in-memory repository state, a responsive `/dashboard` preview, and repository switching; it has no sample action or `/v1` API client. MCP/private/local repository connection has been removed from the active build.
...
- **Target integration:** the browser SHALL call the five-route `/v1` backend contract directly once `TASK-005` lands. The current client shell does not make backend requests.
