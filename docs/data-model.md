...
Conventions: `uuid` is opaque UUIDv4 [assumption]; `sha256-id` is a 64-character content-derived identifier for snapshots, symbols, and structural edges; `datetime` is RFC 3339 UTC; hashes use SHA-256; line numbers are 1-based and columns 0-based.
...
| Field | Type | Null? | Default | Description |
|---|---|---:|---|---|
| `snapshot_id` | sha256-id | no | derived | `hash(repository + commit_sha + analyzer_version)` using canonical domain-separated input encoding |
...
| `snapshot_id` | sha256-id | no | — | Parent |
...
| Field | Type | Null? | Default | Description |
|---|---|---:|---|---|
| `symbol_id` | sha256-id | no | derived | Stable within snapshot from path, kind, name, and declaration position |
| `snapshot_id` | sha256-id | no | — | Parent snapshot |
...
| Field | Type | Null? | Default | Description |
|---|---|---:|---|---|
| `edge_id` | sha256-id | no | derived | Structural edge ID from kind, endpoints, and relationship position |
| `snapshot_id` | sha256-id | no | — | Parent snapshot |
| `edge_type` | enum | no | — | imports, calls, module-import |
| `from_symbol_id`, `to_symbol_id` | sha256-id | no | — | Existing endpoints |
...
| `snapshot_id`, `concept_id` | sha256-id/string | no | — | Snapshot and stable taxonomy key |
| `symbol_ids`, `evidence_span_ids` | sha256-id[]/uuid[] | no | — | Both arrays non-empty |
...
| `session_id`, `snapshot_id` | uuid/sha256-id | no | — | Authorization and source |
