...
|---|---|---:|---|---|
| `session_id` | uuid | no | generated by `POST /v1/analyses` | Opaque ephemeral scope handle; held client-side and resubmitted with the snapshot ID on all later xray/teach-back requests |
| `created_at` | datetime | no | now | Creation time |
| `expires_at` | datetime | no | +30 minutes (1800 seconds) | Hard process-scoped session TTL; matches `EphemeralSessionStore` |
...
| Session, graph, bounded cited excerpts, narratives, questions, responses, learner state, gaps, derivations | Session-confidential | 30 minutes (1800 seconds) from session creation | Client-held; TTL cascade on backend-persisted portions |
