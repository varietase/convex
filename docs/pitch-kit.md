...
| **0:18–0:45** | Zoom from function to its exact caller/callee path; relation labels appear. Open one evidence excerpt. | “Now zoom out. X-Ray shows only connections found by deterministic analysis. This call opens to its exact source evidence. If the analyzer cannot establish a relationship, X-Ray leaves it out.” | Technological Implementation; Design |
...
| **1:42–2:05** | Split-screen diagram: code-evidence graph on left, learner-state graph on right; Cursor/Cody labels outside the product. | “This is not just Cursor or Cody. Those products can navigate, explain, and edit code. X-Ray adds proof-of-comprehension: one graph records what the repository proves; a separate graph records only what this response demonstrated. The graph itself—and MCP—are not our novelty.” | Quality of the Idea |
| **2:05–2:25** | Two-repository architecture: Vercel client/BFF → Hugging Face Docker Space → deterministic analyzer and GPT-5.6. | “The current build stays in two repositories: a Vercel web client and BFF, and a Hugging Face Docker Space with FastAPI, deterministic analysis, minimal LangChain/LangGraph, and GPT-5.6 above the graph.” | Technological Implementation |
...
| **2:36–2:45** | Return to changed gap list and student-builder use case. | “There is no validated payer yet. Next we test whether twenty builders use X-Ray twice—and whether one institution defines a real evaluation. First, make the code explainable by its builder.” | Potential Impact |
...
**Say this first:** “Cursor and Cody help developers navigate, explain, and change code. X-Ray asks whether the human can explain an evidence-backed path, then records what that response demonstrated in a separate learner-state graph.”

| Alternative | What it already does well | X-Ray’s added proof | Claim we refuse |
|---|---|---|---|
| Cursor | Codebase-aware chat and agentic editing | Read-only path evidence → teach-back → changed gap list | That X-Ray should replace an IDE/agent |
...
**If pushed:** “A better explainer can produce better prose; it still does not prove the learner can reproduce the reasoning. X-Ray separates repository truth from demonstrated learner evidence so one cannot overwrite the other.”
...
- The current architecture remains Vercel client/BFF plus Hugging Face Docker Space/FastAPI/LangChain/LangGraph. Future MCP/extension work is F-102, not the current build.
