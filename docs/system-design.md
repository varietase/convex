...
```text
Browser
  | same-origin HTTPS + ephemeral cookie
  v
Vercel web client + BFF/proxy [repository 1]
  | HTTPS + server-held backend credential
  v
Hugging Face Docker Space [repository 2]
  FastAPI -> deterministic Tree-sitter pipeline -> ephemeral evidence store
...
The browser never receives the backend credential or calls the Space directly [assumption]. This removes browser-to-Space CORS and secret exposure while preserving the required two deployments.


## Components & responsibilities
| Component | Owns | Depends on | Features |
|---|---|---|---|
| Web experience | Intake, semantic zoom, graph/list views, teach-back UX, visible provenance, accessibility | Same-origin BFF | F-002–F-005 |
| Vercel BFF/proxy | Ephemeral session, CSRF/origin checks, request bounds, backend credential, response filtering | Space API | F-001–F-005 |
| FastAPI boundary | Typed API, session authorization, rate/bound checks, job status | Backend services | F-001–F-005 |
...
| Ephemeral store/job service | Session-scoped entities, SSE progress, TTL cleanup | Space memory/disk [assumption] | F-001–F-005 |
...
1. Browser creates an ephemeral session through the BFF, then submits a sample ID or public HTTPS repository URL plus immutable revision.
2. BFF validates body/origin/session, attaches its backend credential, and forwards the request.
3. Backend rejects unsupported hosts, mutable/missing revisions, unsafe paths, symlinks, submodules, archives, size/file/line bounds, or unsupported language. Exact MVP bounds are 5 MiB fetched content, 500 files, 100,000 physical lines, and 60 seconds [assumption].
4. Analyzer parses TypeScript/JavaScript source [assumption], emits symbols and only deterministically resolved import/call edges, and attaches one or more file/line spans to every edge.
...
6. Full source files are deleted immediately after parsing. Only bounded cited excerpts, their file/line spans, and hashes persist with the session until TTL [assumption]. SSE reports coarse stages without source text.
...
|---|---|---|---|
| Two repositories: Vercel + HF Docker Space | Current baseline; separates polished web delivery from Python analysis | Two deploys, cold starts, proxy contract | Re-platform during hackathon |
| Same-origin Vercel BFF [assumption] | Keeps backend credential server-side and browser API same-origin | Extra hop and BFF code | Direct browser-to-Space |
...
| SSE stage progress [assumption] | One-way status fits cold/long jobs | Reconnect handling | WebSockets or silent wait |
...
|---|---|---|---|
| Browser → Vercel | HTTPS; signed ephemeral cookie + CSRF for mutations [assumption] | IDs, bounded input, response | Typed 4xx/5xx; retry only idempotent reads |
| Vercel → Space | HTTPS bearer credential held by Vercel [assumption] | Authorized session context and API payload | Timeout/circuit response; sample CTA |
...
## Deployment topology
Vercel serves static/client code and serverless BFF routes. The Hugging Face Docker Space exposes one FastAPI port and contains parsing, reasoning, transient jobs, and the sample artifact. Only the BFF is an authorized backend caller [assumption], though the Space has a public network address. No production database is required for MVP; session loss on restart is an accepted limitation and must be visible.
...
- **Accepted now:** narrow TS/JS support [assumption] and omission over broad low-precision edges.
