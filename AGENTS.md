...
Two repositories form the hackathon baseline: Cloudflare Workers client (`xray-client`, live at `https://convex.varietase.workers.dev`) and Hugging Face Docker Space FastAPI backend (`xray-backend`). The intended live loop calls the backend directly over HTTPS — no BFF/proxy, with a CORS origin allowlist only. Backend uses deterministic Tree-sitter analysis, minimal LangChain/LangGraph, and GPT-5.6 only above the evidence graph. The current client is a public-GitHub-URL-only shell: one shared modal, centralized in-memory repository state, and a `/dashboard` preview; it has no MCP/private/local path and no API client yet. ADR-0002 local-first MCP App is a future recommendation, not active work.
